...
from patents.opensearch_client import get_opensearch_client, create_patents_index, create_papers_index, create_reject_documents_index
        ...
        for patent in patents:
            doc = {
                ...
                '_source': {
                    ...
                    'application_date': patent.application_date,  # 이미 문자열로 저장됨
                    'applicant': patent.applicant,
                    'registration_number': patent.registration_number,
                    'registration_date': patent.registration_date,  # 이미 문자열로 저장됨
...
def migrate_reject_documents_to_opensearch(batch_size=500):
    """
    거절결정서 데이터를 OpenSearch로 마이그레이션
    """
    print("\n" + "="*60)
    print("  거절결정서 데이터 OpenSearch 마이그레이션")
    print("="*60 + "\n")
    ...
    # 인덱스 생성 (이미 존재하면 스킵)
    create_reject_documents_index(client, index_name='reject_documents')

    # PostgreSQL에서 거절결정서 데이터 가져오기
    total_count = RejectDocument.objects.count()
    print(f"📊 총 거절결정서 데이터: {total_count:,}건\n")
    ...
    # 전체 데이터를 배치로 나눠서 처리
    for offset in range(0, total_count, batch_size):
        docs = RejectDocument.objects.all()[offset:offset + batch_size]
        ...
        for doc in docs:
            document = {
                ...
                '_source': {
                    'doc_id': doc.doc_id,
                    'send_number': doc.send_number,
                    'send_date': doc.send_date,
                    'applicant_code': doc.applicant_code,
                    'applicant': doc.applicant,
                    'agent': doc.agent,
                    'application_number': doc.application_number,
                    'invention_name': doc.invention_name,
                    'examination_office': doc.examination_office,
    ...
    # 인덱스 통계
    client.indices.refresh(index='reject_documents')
    stats = client.cat.count(index='reject_documents', format='json')
    print(f"📊 OpenSearch 인덱스: {stats[0]['count']}건\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', choices=['patents', 'papers', 'reject_documents', 'all'], default='all',
                        help='마이그레이션할 데이터 타입 (patents, papers, reject_documents, all)')
    ...
    try:
        ...
        if args.type in ['reject_documents', 'all']:
            migrate_reject_documents_to_opensearch()
