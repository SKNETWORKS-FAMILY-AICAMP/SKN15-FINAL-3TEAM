...
def load_patents_streaming():
    ...
    if existing_count > 0:
        ...
        if response.lower() == 'y':
            cur.execute("DELETE FROM patents")
            conn.commit()
    ...
    try:
        ...
        for chunk_idx, df_chunk in enumerate(pd.read_csv(CSV_FILE, chunksize=chunk_size, index_col=0)):
            ...
            if batch_data:
                ...
                print(f"  ✓ {total_inserted:,}건 적재 완료 (청크 {chunk_idx + 1})")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("   pip install psycopg2-binary tqdm")
        exit(1)

    # 실행 전 DB_CONFIG 확인
    print("\n⚠️  주의: DB_CONFIG에 실제 RDS 정보를 입력했는지 확인하세요!")
